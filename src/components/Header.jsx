...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (lang === "eng") {
      setheaderTitle(data.headerTitle.en);
      setheaderBtn(data.headerBtn.en);
    }
    else if (lang === "spa") {
      setheaderTitle(data.headerTitle.es);
      setheaderBtn(data.headerBtn.es);
    }
    ...
    <div className="w-full h-screen heroSec bg-cover py-20  bg-no-repeat bg-center justify-center flex items-center lg:pt-20 mx-auto ">
      <div className='text-center z-10 pt-2'>
        ...
        <div className="flex justify-center pt-8 pb-4 md:pb-10 lg:pb-16 md:pt-10 lg:pt-14 ">
          <p className='text-base md:text-xl text-white text-center  font-Inter font-medium lg:w-[750px] md:w-[550px] sm:w-[440px]  w-[335px] drop-shadow-9xl'>{headerTitle}</p>
        </div>

        <div className="w-[200px] mx-auto pt-4 md:pt-0 sm:pt-6">
          <a href="#details" className="text-lg text-[#97E4FC] font-normal font-Inter cursor-pointer">{headerBtn}<img src={arrow} alt='' className="w-auto duration-[10s] transition-all pt-6 animate-bounce m-auto" />
          </a>
        </div>
      </div>
    </div>
